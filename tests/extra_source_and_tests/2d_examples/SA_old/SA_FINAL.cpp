...
Real DL3 = 16 * DH;
Real DL = DL1 + DL2 + DL3;
Real resolution_ref = DH / 12.0; // ratio 2: 12 24 48
...
StdVec<Vecd> createRadialObservationPoints(Real full_height = DH, Vecd translation = Vecd(-DL1 - 0.5 * DL2, -0.5 * DH)) 
{
    StdVec<Vecd> observation_points;
    const int n_pts = 31;
    for (int i = 0; i < n_pts; ++i)
    {
        Real y = full_height * i / (n_pts - 1);
        Vecd point_coordinate(4 * resolution_ref, y);
        observation_points.emplace_back(point_coordinate + translation);
    }
    return observation_points;
}
StdVec<Vecd> createLeftRadialFFRObservationPoints(Real full_height = DH, Vecd translation = Vecd(-DH, -0.5 * DH)) 
{
    StdVec<Vecd> observation_points;
    const int n_pts = 31;
    for (int i = 0; i < n_pts; ++i)
...
StdVec<Vecd> createRightRadialFFRObservationPoints(Real full_height = DH, Vecd translation = Vecd(DH, -0.5 * DH))
{
    StdVec<Vecd> observation_points;
    const int n_pts = 31;
    for (int i = 0; i < n_pts; ++i)
...
int main(int ac, char *av[])
{
    ...
    sph_system.setRestartStep(8000);
