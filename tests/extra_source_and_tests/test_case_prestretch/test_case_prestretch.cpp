 ...
 class LinearElasticSolidWithPrestretch : public LinearElasticSolid
{
 ...
 //         pre_strain -= pre_strain.trace() / Dimensions * Matd::Identity(); //减去体积项 否则对于泊松比0.499，lambda0_值更大
 //         return lambda0_ * strain.trace() * Matd::Identity() + 2.0 * G0_ * strain + 2.0 * G0_ * pre_strain; // 试试泊松比0.45且不减去体积项--结果几乎看不见壁面粒子移动
 //     };
 //
 //   private:
 //     Matd F_p_inv;
