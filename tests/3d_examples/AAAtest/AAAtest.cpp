...
class ParticleGenerator<SurfaceParticles, ShellBoundary> : public ParticleGenerator<SurfaceParticles>
{
  ...
  public:
    ...
    {
        const Real radius_mid_surface = fluid_radius + resolution_shell_ * 0.5; // 中层surface半径 比流体多0.5个分辨率
        const auto particle_number_mid_surface =
            int(2.0 * radius_mid_surface * Pi / resolution_shell_); //周长/分辨率--环向粒子数
        const auto particle_number_height =  // 径向粒子数  
            int((full_length + 2.0 * wall_thickness_) / resolution_shell_); // 延长2个wall_thickness_
        for (int i = 0; i < particle_number_mid_surface; i++) // 环向
        {
            for (int j = 0; j < particle_number_height; j++) // 径向
            {
                Real theta = (i + 0.5) * 2 * Pi / (Real)particle_number_mid_surface; // 弧度
                // y-流向，x、z截面方向
                ...
                addPositionAndVolumetricMeasure(Vec3d(x, y, z),
                                                resolution_shell_ * resolution_shell_); // dA -- Volume
                // 法相朝向
...
void poiseuille_flow(const Real resolution_ref, const Real resolution_shell, const Real shell_thickness)
{
    ...
    const Real wall_thickness = resolution_ref * 4.0; // buffer每侧2个分辨率宽度
    ...
    const Vec3d disposer_halfsize(fluid_radius * 1.1, resolution_ref * 2, fluid_radius * 1.1); // 1.1防泄漏
    ...
    shell_boundary.defineAdaptation<SPH::SPHAdaptation>(1.15, resolution_ref / resolution_shell); // 提高分辨率测试
